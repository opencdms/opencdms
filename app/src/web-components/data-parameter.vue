<template>
  <v-container>
      ...
      <v-col :cols="12">
        <v-card>
          ...
          <v-card-text><select-observed-property v-model="selectedProperty"/></v-card-text>
        </v-card>
      </v-col>
      </v-row>
      ...
      <v-row v-if='prop'>
        <v-col :cols="12">
          <v-card>
            <v-card-title>Select from available data for {{ prop.short_name }}</v-card-title>
            
            <v-card-item>
                <v-container>
                  <v-row>
                    <v-col :cols="4"><select-host v-model="selectedHost"/></v-col>
                    <v-col :cols="4">
                      <VueDatePicker
                        :start-date="initDate"
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const host = ref(null)
      const propRepo = useRepo(ObservedProperty);
      const propOptions = computed(() => { return propRepo.all() });
      const prop = ref(null)
      ...
      const plotChart = async () => {
        ...
        const encodedHostId = encodeURIComponent(selectedHost.value.id);
        const encodedObservedPropertyId = encodeURIComponent(prop.value.id);
      ...
      function setDateRange(host_id) {
        //const encodedHostId = encodeURIComponent(host.value.id);
        const encodedHostId = encodeURIComponent(host_id);
        const encodedObservedPropertyId = encodeURIComponent(prop.value.id);
        ...
        console.log(firstDateUrl, lastDateUrl);
      ...
      const fetchRecord = async(identifier) => {
        ...
        prop.value = useRepo(ObservedProperty).where('id',route.params.id).first();
      };
      ...
      onMounted( async() => {
        ...
        watch( (selectedProperty), (data) => {
          router.push('/data/parameter/'+data.id);
        } )
      ...
      return {
        host, hostOptions, selectedHost,
        prop, propOptions, selectedProperty,
        plotChart,
